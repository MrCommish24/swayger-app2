...
async function settleNumericPropCore(supabase, {
  propId,
  cardId,
...
// server/gameday-nfl-catalog.ts
var NFL_TEAM_CATALOG = Object.freeze([
  {
    sport: "football",
    league: "nfl",
    team_code: "BUF",
...
var DISCORD_NFL_SLATE_SUBSCRIPTION_STATUSES = ["active", "paused", "disabled"];
function normalizeDiscordSubscriptionText(value, options = { maxLength: 128 }) {
  if (value === null || value === void 0) {
    return options.required ? null : null;
  }
  if (typeof value !== "string") return null;
  const normalized = value.trim();
  if (!normalized && options.required || normalized.length > options.maxLength || /[\u0000-\u001f\u007f]/.test(normalized)) {
    return null;
  }
  return normalized || null;
}
function isDiscordNflSlateSubscriptionStatus(value) {
  return typeof value === "string" && DISCORD_NFL_SLATE_SUBSCRIPTION_STATUSES.includes(value);
}
...
var WEEKLY_PICK_CARD_DEFAULT_TOTAL_PROMPT = "Total points in the Bonus Game?";
var WEEKLY_PICK_CARD_NUMERIC_MIN = 0;
var WEEKLY_PICK_CARD_NUMERIC_MAX = 200;
...
function weeklyPickCardPropRole(prop) {
  return prop.prop_role ?? "main_matchup";
}
function weeklyPickCardAnswerType(prop) {
  return prop.answer_type ?? "choice";
}
function isRequiredWeeklyPickCardProp(prop) {
...
function buildWeeklyPickCardParticipantScores(participants, picks, props) {
  const mainProps = props.filter(
    (prop) => weeklyPickCardPropRole(prop) === "main_matchup" && isRequiredWeeklyPickCardProp(prop)
  );
  ...
  const actualTotal = bonusTotalProp?.status === "settled" && Number.isInteger(bonusTotalProp.correct_numeric_answer) ? bonusTotalProp.correct_numeric_answer : null;
  return participants.map((participant) => {
    const participantPicks = picks.filter(
      (pick) => pick.participant_id === participant.id
    );
    const mainPicks = participantPicks.filter(
      (pick) => mainPropIds.has(pick.prop_id)
    );
    ...
    const bonusGamePick = bonusGameProp ? participantPicks.find((pick) => pick.prop_id === bonusGameProp.id) : null;
    const bonusTotalPick = bonusTotalProp ? participantPicks.find((pick) => pick.prop_id === bonusTotalProp.id) : null;
    const predictedTotal = Number.isInteger(bonusTotalPick?.numeric_answer) ? bonusTotalPick.numeric_answer : null;
    return {
...
function serializeWeeklyPickCardBonus(props, scores) {
  const bonusGameProp = props.find(
    (prop) => weeklyPickCardPropRole(prop) === "bonus_game"
  );
  const bonusTotalProp = props.find(
  ...
  return {
    ...
    game: bonusGameProp ? {
      ...
      answer_options: Array.isArray(bonusGameProp.answer_options) ? bonusGameProp.answer_options : [],
      line_text: bonusGameProp.line_text ?? null,
      status: bonusGameProp.status ?? "pending",
      correct_answer: bonusGameProp.status === "settled" ? bonusGameProp.correct_answer ?? bonusGameProp.correct_answer_ids?.[0] ?? null : null
    } : null,
    total: bonusTotalProp ? {
      ...
      actual_total: bonusTotalProp.status === "settled" ? bonusTotalProp.correct_numeric_answer ?? null : null
    } : null,
...
function normalizeWeeklyPickCardMatchup(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const raw = value;
...
function normalizeWeeklyPickCardConfig(value, matchupCount) {
  ...
  const bonusGame = bonusEnabled ? normalizeWeeklyPickCardMatchup(rawBonus?.game) : null;
  const bonusLabel = bonusEnabled && !bonusGame ? normalizeWeeklyPickCardText(rawBonus?.label, 180, true) : null;
  const bonusOptions = bonusEnabled && !bonusGame && Array.isArray(rawBonus?.answer_options) ? [...new Set(
  ...
  const rawTotal = rawBonus?.total_prediction && typeof rawBonus.total_prediction === "object" && !Array.isArray(rawBonus.total_prediction) ? rawBonus.total_prediction : null;
  const totalEnabled = bonusEnabled && rawTotal?.enabled === true;
  const totalPrompt = totalEnabled ? normalizeWeeklyPickCardText(
  ...
  if (!weekLabel || !Number.isInteger(minimum) || minimum < 1 || minimum > matchupCount || !scoringMode || hasDeadlineDisplayText && !deadlineDisplayText || bonusEnabled && !bonusGame && (!bonusLabel || bonusOptions.length < 2) || totalEnabled && (!bonusGame || !totalPrompt || totalRule !== "exact")) {
    return null;
  }
  return {
...
function normalizeWeeklyPickCardMatchups(value) {
  ...
  const matchups = value.map((entry) => normalizeWeeklyPickCardMatchup(entry)).filter((matchup) => matchup !== null);
...
function registerGamedayRoutes(app2) {
  ...
  app2.post(
    ...
    async (req, res2) => {
      ...
      if (body.receipt_channel_id !== void 0 && body.receipt_channel_id !== null && !receiptChannelId) {
        res2.status(400).json({ error: "receipt_channel_id must be a valid Discord identifier" });
        return;
      }
      if (body.receipt_channel_name !== void 0 && body.receipt_channel_name !== null && !receiptChannelName) {
        res2.status(400).json({ error: "receipt_channel_name must be valid text" });
        return;
      }
      ...
      if (body.discord_guild_name !== void 0 && body.discord_guild_name !== null && !discordGuildName) {
        res2.status(400).json({ error: "discord_guild_name must be valid text" });
        return;
      }
      if (body.reward_text !== void 0 && body.reward_text !== null && typeof body.reward_text !== "string") {
        res2.status(400).json({ error: "reward_text must be text" });
        return;
      }
      const status = body.status === void 0 || body.status === null ? "active" : typeof body.status === "string" ? body.status.trim().toLowerCase() : body.status;
      ...
      const supabase = getServiceSupabase();
      const { data: subscription, error } = await supabase.from("discord_gameday_subscriptions").upsert(
        {
      ...
      if (error) {
        ...
        res2.status(500).json({ error: "Could not save Discord NFL Slate subscription" });
        return;
      }
  ...
  app2.get(
    ...
    async (req, res2) => {
      ...
      const { data: subscription, error } = await getServiceSupabase().from("discord_gameday_subscriptions").select(
        "id, discord_guild_id, discord_guild_name, game_day_channel_id, game_day_channel_name, receipt_channel_id, receipt_channel_name, reward_text, status, configured_by_discord_user_id, configured_by_discord_user_name, created_at, updated_at"
      ).eq("discord_guild_id", discordGuildId).maybeSingle();
      if (error) {
        ...
        res2.status(500).json({ error: "Could not read Discord NFL Slate subscription" });
        return;
      }
      if (!subscription) {
        res2.status(404).json({ error: "Discord NFL Slate subscription not found" });
  ...
  app2.get(
    ...
    async (req, res2) => {
      if (!isBotApiKeyValid(req)) {
        res2.status(401).json({ ok: false, error: "Valid Game Day bot credentials are required" });
        return;
      }
      const guildId = getRequestedDiscordGuildId(req);
      if (!guildId) {
        res2.status(400).json({ ok: false, error: "X-Discord-Guild-ID is required for Madden participation events" });
        return;
      }
      const rawLimit = req.query.limit;
      if (Array.isArray(rawLimit) || rawLimit !== void 0 && typeof rawLimit !== "string") {
        res2.status(400).json({ ok: false, error: "limit must be provided once" });
        return;
      }
      const limit = rawLimit === void 0 ? 50 : Number(rawLimit);
      if (!Number.isInteger(limit) || limit < 1 || limit > 100) {
        res2.status(400).json({ ok: false, error: "limit must be an integer from 1 to 100" });
        return;
      }
  ...
  app2.post(
    ...
    async (req, res2) => {
      ...
      const supabase = getServiceSupabase();
      const { data: existing, error: existingError } = await supabase.from("gameday_madden_participation_events").select("id, discord_guild_id, delivered_at").eq("id", req.params.eventId).maybeSingle();
      if (existingError) {
        res2.status(503).json({
          ok: false,
          error: "Madden participation events are not enabled; apply the participation migration first",
      ...
      if (!existing) {
        res2.status(404).json({ ok: false, error: "Madden participation event not found" });
        return;
      }
      if (normalizeDiscordGuildId(existing.discord_guild_id) !== guildId) {
        res2.status(403).json({ ok: false, error: "Discord guild is not authorized for this event" });
        return;
      }
      if (existing.delivered_at) {
        res2.json({ ok: true, already: true, delivered_at: existing.delivered_at });
        return;
      }
      const deliveredAt = (/* @__PURE__ */ new Date()).toISOString();
      const { data: acknowledged, error } = await supabase.from("gameday_madden_participation_events").update({ delivered_at: deliveredAt }).eq("id", req.params.eventId).eq("discord_guild_id", guildId).is("delivered_at", null).select("id, delivered_at").maybeSingle();
      if (error) {
        console.error("[gameday] Madden participation event acknowledgement failed:", error.message);
        res2.status(500).json({ ok: false, error: "Could not acknowledge Madden participation event" });
        return;
      }
      if (!acknowledged) {
  ...
  app2.get(
    ...
    async (req, res2) => {
      ...
      if (masterSlateId === null || rawGuildId === null || rawLimit === null) {
        res2.status(400).json({ ok: false, error: "Query parameters must be provided once" });
        return;
      }
      if (masterSlateId !== void 0 && !isUuidLike(masterSlateId.trim())) {
        res2.status(400).json({ ok: false, error: "master_slate_id must be a valid UUID" });
        return;
      }
      const discordGuildId = rawGuildId === void 0 ? void 0 : normalizeDiscordGuildId(rawGuildId);
      if (rawGuildId !== void 0 && !discordGuildId) {
        res2.status(400).json({ ok: false, error: "discord_guild_id must be a valid guild ID" });
        return;
      }
      const limit = rawLimit === void 0 ? 50 : parsePositiveInteger(rawLimit);
      if (!limit || limit > 100) {
        res2.status(400).json({ ok: false, error: "limit must be an integer between 1 and 100" });
        return;
      }
      const supabase = getServiceSupabase();
      let query = supabase.from("nfl_weekly_slate_room_instances").select("*").eq("status", "post_ready").order("created_at", { ascending: true }).limit(limit);
      if (masterSlateId !== void 0) query = query.eq("master_slate_id", masterSlateId.trim());
      if (discordGuildId !== void 0) query = query.eq("discord_guild_id", discordGuildId);
      ...
      if (error) {
        ...
        if (isMissingWeeklySlateSchemaError(error)) {
          ...
          res2.status(500).json({ ok: false, error: "Could not load post-ready NFL weekly slate instances" });
        }
        return;
      }
      const rows = data ?? [];
      const guildIds = [...new Set(rows.map((row) => row.discord_guild_id).filter(Boolean))];
      const guildNameById = /* @__PURE__ */ new Map();
      if (guildIds.length > 0) {
        const { data: subscriptions, error: subscriptionError } = await supabase.from("discord_gameday_subscriptions").select("discord_guild_id, discord_guild_name").in("discord_guild_id", guildIds);
        if (subscriptionError) {
          console.error("[gameday] weekly slate delivery guild lookup error:", subscriptionError.message);
          res2.status(500).json({ ok: false, error: "Could not load Discord guild names for slate instances" });
          return;
        }
      ...
      const instances = rows.map(
        (row) => serializeWeeklySlateRoomInstance({
          ...row,
          discord_guild_name: guildNameById.get(row.discord_guild_id) ?? row.post_payload?.discord_guild_name ?? null
        })
      );
      res2.json({ ok: true, count: instances.length, instances });
    }
  );
  async function updateWeeklySlateDeliveryStatus(req, res2, nextStatus) {
    if (!isBotApiKeyValid(req)) {
      res2.status(401).json({ ok: false, error: "Valid Game Day bot credentials are required" });
      return;
    }
    ...
    if (!instanceId || !isUuidLike(instanceId)) {
      res2.status(400).json({ ok: false, error: "instanceId must be a valid UUID" });
      return;
    }
    const body = req.body ?? {};
    const now = (/* @__PURE__ */ new Date()).toISOString();
    ...
    if (nextStatus === "posted") {
      ...
      if (!discordMessageId) {
        res2.status(400).json({ ok: false, error: "discord_message_id is required" });
        return;
      }
      update = {
        status: "posted",
      ...
      const postError = normalizeWeeklySlateText(body.post_error, 2e3);
      if (!postError) {
        res2.status(400).json({ ok: false, error: "post_error is required and must be at most 2000 characters" });
        return;
      }
      update = {
        status: "failed",
    ...
    const supabase = getServiceSupabase();
    const { data: instance, error } = await supabase.from("nfl_weekly_slate_room_instances").update(update).eq("id", instanceId).eq("status", "post_ready").select("*").maybeSingle();
    if (error) {
      ...
      if (isMissingWeeklySlateSchemaError(error)) {
        ...
        res2.status(500).json({ ok: false, error: `Could not mark weekly slate instance ${nextStatus}` });
      }
      return;
    }
    if (instance) {
      res2.json({ ok: true, instance: serializeWeeklySlateRoomInstance(instance) });
      return;
    }
    const { data: current, error: currentError } = await supabase.from("nfl_weekly_slate_room_instances").select("id, status").eq("id", instanceId).maybeSingle();
    if (currentError) {
      res2.status(500).json({ ok: false, error: "Could not verify weekly slate instance state" });
      return;
    }
    if (!current) {
      res2.status(404).json({ ok: false, error: "NFL weekly slate room instance not found" });
      return;
    }
  ...
  app2.post(
    ...
    async (req, res2) => {
      await updateWeeklySlateDeliveryStatus(req, res2, "posted");
    }
  );
  app2.post(
    "/api/gameday/bot/nfl-weekly-slate-room-instances/:instanceId/failed",
    async (req, res2) => {
      await updateWeeklySlateDeliveryStatus(req, res2, "failed");
    }
  );
  ...
  app2.get("/api/gameday/catalog/teams", (req, res2) => {
    if (!isBotApiKeyValid(req)) {
      res2.status(401).json({ error: "Valid Game Day bot credentials are required" });
      return;
    }
    ...
    if (league !== "nfl") {
      res2.status(400).json({ error: "league must be nfl" });
      return;
    }
  ...
  app2.post("/api/gameday/rooms", async (req, res2) => {
    ...
    if (isWeeklyPickCard) {
      ...
      if (weeklyConfig.bonus.enabled) {
        ...
        props.push({
          card_id: weeklyCard.id,
          question: bonusGame ? `Bonus Game: Who wins: ${bonusGame.team_a} vs ${bonusGame.team_b}?` : weeklyConfig.bonus.label ?? "Bonus Game",
          answer_options: bonusGame ? [bonusGame.team_a, bonusGame.team_b] : weeklyConfig.bonus.answer_options,
...
${publicLink2}`;
  ...
  app2.get(
    ...
    async (req, res2) => {
      ...
      if (participant && allPropIds.length > 0) {
        const { data: picks } = await supabase.from("gameday_picks").select("prop_id, selected_answer, numeric_answer").eq("participant_id", participant.id).in("prop_id", allPropIds);
        ...
        myPickDetails = Object.fromEntries(
          (picks ?? []).map((p) => [
            p.prop_id,
            {
              ...
              numeric_answer: Number.isInteger(p.numeric_answer) ? p.numeric_answer : null
            }
          ])
        );
      }
      ...
      const sanitizedCards = cards.map((card) => ({
        ...
        gameday_props: (card.gameday_props ?? []).map((prop) => ({
          ...
          correct_numeric_answer: prop.status === "settled" ? prop.correct_numeric_answer ?? null : null
        }))
      }));
  ...
  app2.post(
    ...
    async (req, res2) => {
      ...
      const supabase = getServiceSupabase();
      const { data: prop } = await supabase.from("gameday_props").select("*, gameday_pick_cards(id, status, room_id, scheduled_lock_at, gameday_rooms(archived_at, status, source, sport, template_type, discord_guild_id, discord_channel_id))").eq("id", propId).single();
      if (!prop) {
      ...
      const answerType = prop.answer_type ?? "choice";
      let selectedAnswer;
      let numericAnswer = null;
      if (answerType === "integer") {
        const min = Number.isInteger(prop.numeric_min) ? prop.numeric_min : WEEKLY_PICK_CARD_NUMERIC_MIN;
        const max = Number.isInteger(prop.numeric_max) ? prop.numeric_max : WEEKLY_PICK_CARD_NUMERIC_MAX;
        if (typeof requestedNumericAnswer !== "number" || !Number.isInteger(requestedNumericAnswer) || requestedNumericAnswer < min || requestedNumericAnswer > max) {
          res2.status(400).json({
        ...
        if (typeof requestedSelectedAnswer !== "string" || requestedSelectedAnswer.length === 0) {
      ...
      if (userId) {
        ...
        const { data } = await supabase.from("gameday_participants").select("*").eq("room_id", roomId).eq("guest_session_id", guestSessionId).maybeSingle();
        participant = data;
      }
      if (!participant) {
      ...
      const cardRoom = prop.gameday_pick_cards?.gameday_rooms;
      const isDiscordMaddenWeekly = cardRoom?.source === "discord" && cardRoom?.sport === "madden" && cardRoom?.template_type === "weekly_pick_card" && !!cardRoom?.discord_guild_id && !!cardRoom?.discord_channel_id;
      let pick = null;
      let error = null;
      ...
      if (isDiscordMaddenWeekly) {
        ...
        const direct = await supabase.from("gameday_picks").upsert(
          {
            prop_id: propId,
            participant_id: participant.id,
      ...
      if (error) {
        ...
        res2.status(500).json({
          error: isDiscordMaddenWeekly ? `Could not save Madden pick and participation event: ${error.message ?? "unknown database error"}` : `Could not save pick: ${error.message ?? "unknown database error"}`
        });
        return;
      }
      if (!pick) {
        res2.status(500).json({ error: "Could not save Madden pick and participation event" });
        return;
      }
  ...
  app2.get(
    ...
    async (req, res2) => {
      ...
      const { data: cards } = await supabase.from("gameday_pick_cards").select("id, phase, status, display_order, gameday_props(id, question, answer_options, line_text, prop_role, answer_type, numeric_min, numeric_max, correct_answer, correct_answer_ids, correct_numeric_answer, status, display_order)").eq("room_id", roomId).order("display_order", { ascending: true });
      const card = (cards ?? []).find((item) => item.phase === "pregame") ?? (cards ?? [])[0];
      if (!card) {
        res2.status(404).json({ error: "No playable card found for settlement" });
        return;
      }
      const settlementItems = [...card?.gameday_props ?? []].filter((prop) => isRequiredWeeklyPickCardProp(prop)).sort((a, b) => (a.display_order ?? 0) - (b.display_order ?? 0)).map((prop, index) => ({
        prop_id: prop.id,
        ...
        actual_total: prop.status === "settled" && (prop.answer_type ?? "choice") === "integer" ? prop.correct_numeric_answer ?? null : null
      }));
      const matchups = settlementItems.filter(
        (prop) => prop.answer_type === "choice"
      );
      const bonusGame = settlementItems.find(
        (prop) => prop.prop_role === "bonus_game"
      ) ?? null;
      const bonusTotal = settlementItems.find(
        (prop) => prop.prop_role === "bonus_total"
      ) ?? null;
      const settled = settlementItems.filter(
        (prop) => prop.status === "settled"
      ).length;
  ...
  app2.patch(
    ...
    async (req, res2) => {
      ...
      const answerType = prop.answer_type ?? "choice";
      if (answerType === "integer") {
        const requestedActualTotal = req.body?.actual_total ?? req.body?.correct_numeric_answer;
        const min = Number.isInteger(prop.numeric_min) ? prop.numeric_min : WEEKLY_PICK_CARD_NUMERIC_MIN;
        const max = Number.isInteger(prop.numeric_max) ? prop.numeric_max : WEEKLY_PICK_CARD_NUMERIC_MAX;
        if (typeof requestedActualTotal !== "number" || !Number.isInteger(requestedActualTotal) || requestedActualTotal < min || requestedActualTotal > max) {
          res2.status(400).json({
            error: `actual_total must be an integer between ${min} and ${max}`
          });
          return;
        }
        const priorNumeric = Number.isInteger(prop.correct_numeric_answer) ? prop.correct_numeric_answer : null;
        const corrected2 = priorNumeric !== null && priorNumeric !== requestedActualTotal;
        const { data: propPicks2 } = await supabase.from("gameday_picks").select("participant_id").eq("prop_id", propId);
        const participantsUpdated2 = corrected2 || priorNumeric === null ? new Set(
          (propPicks2 ?? []).map((pick) => pick.participant_id).filter(Boolean)
        ).size : 0;
        await settleNumericPropCore(supabase, {
          propId,
          cardId: card.id,
          correctNumericAnswer: requestedActualTotal
        });
        if (priorNumeric === null || corrected2) {
          await logEvent(
            supabase,
            card?.room_id,
            null,
            operator.hostId,
            corrected2 ? "prop_settlement_corrected" : "prop_settled",
            {
              prop_id: propId,
              card_id: card?.id,
              phase: card?.phase,
              prop_role: prop.prop_role ?? "bonus_total",
              answer_type: "integer",
              prior_actual_total: corrected2 ? priorNumeric : null,
              actual_total: requestedActualTotal,
              operator: operator.kind
            }
          );
        }
        const { data: progressProps2 } = await supabase.from("gameday_props").select("id, status, answer_options, answer_type").eq("card_id", card.id);
        const requiredProps = (progressProps2 ?? []).filter(
          (item) => isRequiredWeeklyPickCardProp(item)
        );
        const total2 = requiredProps.length;
        const settled2 = requiredProps.filter(
        ...
        res2.json({
          ...
          settled_matchups: settled2,
          remaining_matchups: Math.max(0, total2 - settled2),
          all_settled: total2 > 0 && settled2 === total2,
          progress: {
            settled: settled2,
            total: total2,
            remaining: Math.max(0, total2 - settled2),
            complete: total2 > 0 && settled2 === total2
          },
          participants_updated: participantsUpdated2
        });
        return;
      }
      ...
      const { data: progressProps } = await supabase.from("gameday_props").select("id, status, answer_options, answer_type").eq("card_id", card.id);
  ...
  app2.patch(
    ...
    async (req, res2) => {
      ...
      if (room.source === "discord" && room.sport === "madden" && room.template_type === "weekly_pick_card") {
        const { data: cards } = await supabase.from("gameday_pick_cards").select("id, gameday_props(status, answer_options, answer_type)").eq("room_id", roomId);
        const playable = (cards ?? []).flatMap(
          (card) => (card.gameday_props ?? []).filter(
            (prop) => isRequiredWeeklyPickCardProp(prop)
          )
        );
  ...
  app2.get(
    ...
    async (req, res2) => {
      ...
      const { data: standingCards } = await supabase.from("gameday_pick_cards").select("id, gameday_props(id, question, status, answer_options, line_text, prop_role, answer_type, numeric_min, numeric_max, correct_answer, correct_answer_ids, correct_numeric_answer, display_order)").eq("room_id", roomId);
      const playableProps = (standingCards ?? []).flatMap(
        (card) => (card.gameday_props ?? []).filter(
          (prop) => isRequiredWeeklyPickCardProp(prop)
        )
      );
      const scoringProps = isMaddenWeekly ? playableProps.filter(
        (prop) => weeklyPickCardPropRole(prop) === "main_matchup"
      ) : playableProps;
      const totalRequiredProps = scoringProps.length;
      const allRequiredPropsSettled = totalRequiredProps > 0 && scoringProps.every((prop) => prop.status === "settled");
      const { data: participants } = await supabase.from("gameday_participants").select("id, display_name, is_guest").eq("room_id", roomId);
      if (!participants?.length) {
        res2.json({
          leaderboard: [],
          scoring_mode: scoringMode,
          ...isMaddenWeekly ? { bonus: serializeWeeklyPickCardBonus(playableProps, []) } : {}
        });
        return;
      }
      const participantIds = participants.map((p) => p.id);
      const { data: allPicks } = playableProps.length > 0 ? await supabase.from("gameday_picks").select("participant_id, prop_id, selected_answer, numeric_answer, is_correct").in("participant_id", participantIds).in("prop_id", playableProps.map((prop) => prop.id)) : { data: [] };
      const scores = isMaddenWeekly ? buildWeeklyPickCardParticipantScores(
        participants,
        allPicks ?? [],
        playableProps
      ) : participants.map((p) => {
        const myPicks = (allPicks ?? []).filter(
          (pk) => pk.participant_id === p.id && scoringProps.some((prop) => prop.id === pk.prop_id)
        );
      ...
      res2.json({
        leaderboard,
        scoring_mode: scoringMode,
        ...isMaddenWeekly ? { bonus: serializeWeeklyPickCardBonus(playableProps, leaderboard) } : {}
      });
    }
  );
  app2.get(
    ...
    async (req, res2) => {
      ...
      const totalRequiredProps = scoringProps.length;
      const allRequiredPropsSettled = totalRequiredProps > 0 && scoringProps.every((prop) => prop.status === "settled");
      const allPropsSettled = playableProps.length > 0 && playableProps.every((prop) => prop.status === "settled");
      let leaderboard = [];
...
function getWeeklyMoment(templatePropId) {
...
function registerFantasyRoutes(app2) {
  ...
  function buildAnswerOptions(targetType2, seasonMembers, teams, staticOptions, supportsNoOne = false, useTeamNamesForMembers = false) {
  ...
  app2.get(
    ...
    async (req, res2) => {
      ...
      const { data: nextWeekRoom } = await supabase.from("gameday_rooms").select("id, status, week_number, reward_amount_display").eq("league_season_id", seasonId).eq("competition_type", "weekly").eq("experience_type", "fantasy").eq("week_number", wn + 1).is("archived_at", null).maybeSingle();
      ...
      res2.json({
        ...
        next_week: {
          ...
          reward_amount_display: nextWeekPublished ? nextWeekRoom.reward_amount_display ?? null : null
        },
        next_week_number: wn + 1,
        next_week_published: nextWeekPublished,
        next_week_reward_amount_display: nextWeekPublished ? nextWeekRoom.reward_amount_display ?? null : null
      });
    }
  );
  app2.post(
...
var OFFER_DISCLOSURE = "Affiliate disclosure: Swayger may earn a commission if you use this link.";
var TARGET_TYPES = /* @__PURE__ */ new Set(["fantasy_weekly", "game_day"]);
...
function targetType(value) {
  return typeof value === "string" && TARGET_TYPES.has(value) ? value : null;
}
function normalizeOfferInput(input) {
  if (!input || input.provider !== "impact") {
    return { error: "Only normalized Impact offers are supported." };
  }
  const resourceType = input.resource_type === "deal" || input.resource_type === "ad" ? input.resource_type : null;
  ...
  return {
    ...
    tracking_url: boundedString(input.tracking_url, 4e3),
    landing_page_url: boundedString(input.landing_page_url, 4e3),
    creative_url: boundedString(input.creative_url, 4e3),
...
async function getEligibleRoom(supabase, type, id) {
  let query = supabase.from("gameday_rooms").select("id, room_name, room_code, team_a_name, team_b_name, game_date, status, archived_at, experience_type, competition_type, week_number, league_season_id").eq("id", id).in("status", ["draft", "active"]).is("archived_at", null);
  query = type === "fantasy_weekly" ? query.eq("experience_type", "fantasy").eq("competition_type", "weekly") : query.eq("experience_type", "game_day");
  const { data, error } = await query.maybeSingle();
  if (error) throw error;
  return data;
}
async function listEligibleTargets(supabase, type) {
  let query = supabase.from("gameday_rooms").select("id, room_name, room_code, team_a_name, team_b_name, game_date, status, archived_at, experience_type, competition_type, week_number, league_season_id").in("status", ["draft", "active"]).is("archived_at", null).order("created_at", { ascending: false }).limit(100);
  query = type === "fantasy_weekly" ? query.eq("experience_type", "fantasy").eq("competition_type", "weekly") : query.eq("experience_type", "game_day");
  const { data: rooms, error } = await query;
  if (error) throw error;
  const roomIds = (rooms ?? []).map((room) => room.id);
  const { data: participants } = roomIds.length ? await supabase.from("gameday_participants").select("room_id").in("room_id", roomIds) : { data: [] };
  const counts = /* @__PURE__ */ new Map();
  for (const participant of participants ?? []) {
    counts.set(participant.room_id, (counts.get(participant.room_id) ?? 0) + 1);
  }
  const { data: activeAssignments } = roomIds.length ? await supabase.from("swayger_commercial_assignments").select("id, target_id, offer_id, status").eq("target_type", type).eq("status", "published").in("target_id", roomIds) : { data: [] };
...
async function getAssignment(supabase, assignmentId) {
  const { data: assignment, error } = await supabase.from("swayger_commercial_assignments").select("*").eq("id", assignmentId).maybeSingle();
  if (error) throw error;
  if (!assignment) return null;
  const { data: offer } = await supabase.from("swayger_commercial_offers").select("*").eq("id", assignment.offer_id).maybeSingle();
  const room = validUuid(assignment.target_id) ? await getEligibleRoom(supabase, assignment.target_type, assignment.target_id) : null;
...
function registerCommercialRoutes(app2) {
  app2.get("/api/admin/commercial/offers", async (req, res2) => {
    if (!requireAdmin3(req, res2)) return;
    try {
      const supabase = getServiceSupabase();
      const { data, error } = await supabase.from("swayger_commercial_offers").select("*").neq("status", "archived").order("updated_at", { ascending: false }).limit(100);
      if (error) throw error;
      res2.json({ ok: true, offers: (data ?? []).map(safeOffer) });
    } catch (error) {
      console.error("[commercial] offers list failed:", error instanceof Error ? error.message : "unknown");
      res2.status(503).json({ ok: false, error: "Commercial offers are unavailable." });
    }
  });
  app2.post("/api/admin/commercial/offers", async (req, res2) => {
    if (!requireAdmin3(req, res2)) return;
    const normalized = normalizeOfferInput(req.body?.item);
    if ("error" in normalized) {
      res2.status(400).json({ ok: false, error: normalized.error });
      return;
    }
    try {
      const supabase = getServiceSupabase();
      const existingQuery = supabase.from("swayger_commercial_offers").select("*").eq("provider", normalized.provider).eq("source_resource_type", normalized.source_resource_type);
      const existing = normalized.source_external_id ? await existingQuery.eq("source_external_id", normalized.source_external_id).maybeSingle() : { data: null, error: null };
      if (existing.error) throw existing.error;
      if (existing.data) {
        const { data: data2, error: error2 } = await supabase.from("swayger_commercial_offers").update({ ...normalized, status: existing.data.status === "archived" ? "saved" : existing.data.status }).eq("id", existing.data.id).select("*").single();
        if (error2) throw error2;
        res2.json({ ok: true, already_saved: true, offer: safeOffer(data2) });
        return;
      }
      const { data, error } = await supabase.from("swayger_commercial_offers").insert(normalized).select("*").single();
      if (error) throw error;
      res2.status(201).json({ ok: true, already_saved: false, offer: safeOffer(data) });
    } catch (error) {
      console.error("[commercial] offer save failed:", error instanceof Error ? error.message : "unknown");
      res2.status(503).json({ ok: false, error: "Could not save the Swayger Offer." });
    }
  });
  app2.get("/api/admin/commercial/targets", async (req, res2) => {
    if (!requireAdmin3(req, res2)) return;
    const type = targetType(req.query.target_type);
    if (!type) {
      res2.status(400).json({ ok: false, error: "target_type must be fantasy_weekly or game_day." });
      return;
    }
    try {
      const targets = await listEligibleTargets(getServiceSupabase(), type);
      res2.json({ ok: true, targets });
    } catch (error) {
      console.error("[commercial] targets list failed:", error instanceof Error ? error.message : "unknown");
      res2.status(503).json({ ok: false, error: "Commercial targets are unavailable." });
    }
  });
  app2.get("/api/admin/commercial/assignments", async (req, res2) => {
    if (!requireAdmin3(req, res2)) return;
    try {
      const supabase = getServiceSupabase();
      const { data, error } = await supabase.from("swayger_commercial_assignments").select("*").order("updated_at", { ascending: false }).limit(200);
      if (error) throw error;
      const assignments = [];
      for (const assignment of data ?? []) assignments.push(await getAssignment(supabase, assignment.id));
      res2.json({ ok: true, assignments: assignments.filter(Boolean) });
    } catch (error) {
      console.error("[commercial] assignments list failed:", error instanceof Error ? error.message : "unknown");
      res2.status(503).json({ ok: false, error: "Commercial assignments are unavailable." });
    }
  });
  app2.post("/api/admin/commercial/assignments", async (req, res2) => {
    if (!requireAdmin3(req, res2)) return;
    ...
    if (!type || !validUuid(offerId) || !validUuid(targetId)) {
      res2.status(400).json({ ok: false, error: "A valid offer, target type, and target are required." });
      return;
    }
    try {
      const supabase = getServiceSupabase();
      ...
      if (!room) {
        res2.status(409).json({ ok: false, error: "That target is no longer eligible for commercial placement." });
        return;
      }
      const { data: offer } = await supabase.from("swayger_commercial_offers").select("id, status").eq("id", offerId).maybeSingle();
      if (!offer || offer.status === "archived") {
        res2.status(404).json({ ok: false, error: "Swayger Offer not found." });
        return;
      }
      const { data: existing } = await supabase.from("swayger_commercial_assignments").select("*").eq("offer_id", offerId).eq("target_type", type).eq("target_id", targetId).maybeSingle();
      if (existing) {
        const assignment = await getAssignment(supabase, existing.id);
        res2.json({ ok: true, already_assigned: true, assignment });
        return;
      }
      const { data, error } = await supabase.from("swayger_commercial_assignments").insert({ offer_id: offerId, target_type: type, target_id: targetId, status: "draft" }).select("id").single();
      if (error) throw error;
      res2.status(201).json({ ok: true, already_assigned: false, assignment: await getAssignment(supabase, data.id) });
    } catch (error) {
      console.error("[commercial] assignment save failed:", error instanceof Error ? error.message : "unknown");
      res2.status(503).json({ ok: false, error: "Could not assign the Swayger Offer." });
    }
  });
  app2.post("/api/admin/commercial/assignments/:assignmentId/publish", async (req, res2) => {
    if (!requireAdmin3(req, res2)) return;
    const { assignmentId } = req.params;
    if (!validUuid(assignmentId)) {
      res2.status(400).json({ ok: false, error: "Invalid assignment." });
      return;
    }
    try {
      const supabase = getServiceSupabase();
      const { data: current } = await supabase.from("swayger_commercial_assignments").select("*").eq("id", assignmentId).maybeSingle();
      if (!current) {
        res2.status(404).json({ ok: false, error: "Assignment not found." });
        return;
      }
      const room = await getEligibleRoom(supabase, current.target_type, current.target_id);
      if (!room) {
        res2.status(409).json({ ok: false, error: "That target is no longer eligible for publishing." });
        return;
      }
      if (current.status === "published") {
        res2.json({ ok: true, already_published: true, assignment: await getAssignment(supabase, assignmentId) });
        return;
      }
      const { data, error } = await supabase.from("swayger_commercial_assignments").update({ status: "published", published_at: (/* @__PURE__ */ new Date()).toISOString(), updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", assignmentId).select("id").single();
      if (error) {
        if (error.code === "23505") {
          res2.status(409).json({ ok: false, error: "This placement target already has a published offer." });
          return;
        }
        throw error;
      }
      res2.json({ ok: true, already_published: false, assignment: await getAssignment(supabase, data.id) });
    } catch (error) {
      console.error("[commercial] assignment publish failed:", error instanceof Error ? error.message : "unknown");
      res2.status(503).json({ ok: false, error: "Could not publish the assignment." });
    }
  });
  app2.post("/api/admin/commercial/assignments/:assignmentId/unpublish", async (req, res2) => {
    if (!requireAdmin3(req, res2)) return;
    const { assignmentId } = req.params;
    if (!validUuid(assignmentId)) {
      res2.status(400).json({ ok: false, error: "Invalid assignment." });
    ...
    try {
      const supabase = getServiceSupabase();
      const { data, error } = await supabase.from("swayger_commercial_assignments").update({ status: "unpublished", published_at: null, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", assignmentId).select("id").single();
      if (error || !data) {
        res2.status(404).json({ ok: false, error: "Assignment not found." });
        return;
      }
      res2.json({ ok: true, assignment: await getAssignment(supabase, data.id) });
    } catch (error) {
      console.error("[commercial] assignment unpublish failed:", error instanceof Error ? error.message : "unknown");
      res2.status(503).json({ ok: false, error: "Could not unpublish the assignment." });
    }
  });
  app2.get("/api/commercial/placements/:targetType/:targetId", async (req, res2) => {
    const type = targetType(req.params.targetType);
    const targetId = req.params.targetId;
    if (!type || !validUuid(targetId)) {
      res2.json({ ok: true, offer: null });
      return;
    }
    try {
      const supabase = getServiceSupabase();
      ...
      const { data: assignment } = await supabase.from("swayger_commercial_assignments").select("id, offer_id, target_type, target_id, status").eq("target_type", type).eq("target_id", targetId).eq("status", "published").maybeSingle();
      if (!assignment) {
        res2.json({ ok: true, offer: null });
        return;
      }
      const now = (/* @__PURE__ */ new Date()).toISOString();
      const { data: offer } = await supabase.from("swayger_commercial_offers").select("*").eq("id", assignment.offer_id).eq("status", "saved").or(`start_at.is.null,start_at.lte.${now}`).or(`end_at.is.null,end_at.gte.${now}`).maybeSingle();
      res2.json({
        ok: true,
        offer: offer ? {
          ...safeOffer(offer),
...
function registerFantasyReceiptShortLink(app2) {
  app2.get("/r/:shortCode/preview.svg", async (req, res2) => {
    ...
    try {
      const supabase = getServiceSupabase();
      const { data: alias } = await supabase.from("fantasy_weekly_receipt_aliases").select("league_season_id, week_number").eq("short_code", shortCode).maybeSingle();
      if (!alias) {
        res2.status(404).send("Receipt not found");
        return;
      }
      const { data: season } = await supabase.from("fantasy_league_seasons").select("id, league_id").eq("id", alias.league_season_id).maybeSingle();
      if (!season) {
        res2.status(404).send("Receipt not found");
        return;
      }
      ...
      res2.setHeader("Content-Type", "image/svg+xml; charset=utf-8");
      res2.setHeader("Cache-Control", "public, max-age=300");
      res2.send(renderWeeklyReceiptPreviewSvg(preview));
    } catch (error) {
      console.error("[fantasy-receipt-short-link] preview image failed:", error);
      res2.status(500).send("Unable to render receipt preview");
    }
  });
  app2.get("/r/:shortCode", async (req, res2) => {
    ...
    try {
      ...
      const canonicalPath = `/fantasy/weeks/${weeklySeason.league_id}/${weeklySeason.id}/${weeklyAlias.week_number}/receipt`;
      if (isReceiptPreviewCrawler(String(req.headers["user-agent"] ?? ""))) {
        const preview = await loadWeeklyReceiptPreview(
          supabase,
          weeklySeason.id,
          weeklySeason.league_id,
          weeklyAlias.week_number
        );
        if (!preview) {
          res2.status(404).send("Receipt not found");
          return;
        ...
        res2.setHeader("Content-Type", "text/html; charset=utf-8");
        res2.setHeader("Cache-Control", "public, max-age=300");
        res2.send(renderWeeklyReceiptPreviewHtml(
          preview,
          shortUrl,
      ...
      res2.setHeader("Cache-Control", "no-store");
      res2.redirect(302, canonicalPath);
    } catch (error) {
