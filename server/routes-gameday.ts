...
function normalizeWeeklySlateCandidates(
  body: Record<string, unknown>,
  partial = false,
): { values: Record<WeeklySlateCandidateField, string[]>; error?: string } {
  const values = {} as Record<WeeklySlateCandidateField, string[]>;
...
function serializeWeeklySlate(row: any) {
  return {
    id: row.id,
...
function isMissingWeeklySlateSchemaError(error: any): boolean {
  return ["42P01", "PGRST204", "PGRST205"].includes(error?.code);
}

function getWeeklySlateAdminIdentity(req: Request): {
  userId: string | null;
  email: string | null;
} {
  ...
  return {
    userId: /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(userId)
      ? userId
      : null,
    email,
  };
...
function normalizeWeeklySlatePublishGuildIds(value: unknown): {
  provided: boolean;
  ids: string[];
  error?: string;
} {
  if (value === undefined) return { provided: false, ids: [] };
...
function serializeWeeklySlateRoomInstance(row: any) {
  return {
    id: row.id,
...
function deriveMatchupTeams(matchups: string[]): string[] {
  return [...new Set(
...
type SundaySlateRoomInput = {
  roomName: string;
  slateConfig: SundaySlateConfig;
  discordGuildId: string;
  discordChannelId: string;
  gameDate?: string | null;
};

type SundaySlateRoomResult = {
  room: any;
...
/**
 * Creates the same private NFL Sunday Slate room shape used by the Discord
 * room route, without making an HTTP request or coupling the admin publisher
 * to Express. The publisher deliberately stops at room/instance persistence;
 * Discord delivery is a separate bot concern.
 */
async function createPrivateSundaySlateRoom(
  supabase: ReturnType<typeof getServiceSupabase>,
  ...
  try {
    for (const cardDef of cardPhases) {
      ...
      const templateProps = NFL_SUNDAY_SLATE_TEMPLATE.filter(
        (prop) => prop.phase === cardDef.phase &&
          NFL_SUNDAY_SLATE_DEFAULT_PROP_IDS.includes(prop.id),
      );
...
export function registerGamedayRoutes(app: Express) {
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!acknowledged) {
        const { data: deliveredEvent, error: rereadError } = await supabase
          .from("gameday_madden_participation_events")
          .select("delivered_at")
          .eq("id", req.params.eventId)
          .eq("discord_guild_id", guildId)
        ...
        if (rereadError || !deliveredEvent?.delivered_at) {
          ...
          res.status(500).json({ ok: false, error: "Could not confirm Madden participation event acknowledgement" });
          return;
        }
        res.json({ ok: true, already: true, delivered_at: deliveredEvent.delivered_at });
        return;
      }
  ...
  function weeklySlateDbError(res: Response, error: any, fallback: string): boolean {
    ...
    if (isMissingWeeklySlateSchemaError(error)) {
      ...
      res.status(500).json({ ok: false, error: fallback });
    }
    return true;
  }

  app.post("/api/gameday/admin/nfl-weekly-slates", async (req: Request, res: Response) => {
    if (!checkWeeklySlateAdmin(req, res)) return;
    const body = (req.body ?? {}) as Record<string, unknown>;
    ...
    if (!seasonYear) {
      res.status(400).json({ ok: false, error: "season_year is required and must be a positive integer" });
      return;
    }
    if (!weekNumber) {
      res.status(400).json({ ok: false, error: "week_number is required and must be a positive integer" });
      return;
    }
    if (!slateName) {
      res.status(400).json({ ok: false, error: "slate_name is required" });
      return;
    }
    const slateLabel =
      body.slate_label === undefined || body.slate_label === null
        ? null
        : normalizeWeeklySlateText(body.slate_label, 200);
    if (body.slate_label !== undefined && body.slate_label !== null && !slateLabel) {
      res.status(400).json({ ok: false, error: "slate_label must be valid text" });
      return;
    }
    const candidates = normalizeWeeklySlateCandidates(body);
    if (candidates.error) {
      res.status(400).json({ ok: false, error: candidates.error });
      return;
    }
    ...
    const { data, error } = await getServiceSupabase()
      ...
      .single();
    if (weeklySlateDbError(res, error, "Could not create NFL weekly master slate")) return;
    res.status(201).json({ ok: true, slate: serializeWeeklySlate(data) });
  });

  app.get("/api/gameday/admin/nfl-weekly-slates", async (req: Request, res: Response) => {
    if (!checkWeeklySlateAdmin(req, res)) return;
    const seasonYear =
      req.query.season_year === undefined ? null : parsePositiveInteger(req.query.season_year);
    const weekNumber =
      req.query.week_number === undefined ? null : parsePositiveInteger(req.query.week_number);
    if (req.query.season_year !== undefined && !seasonYear) {
      res.status(400).json({ ok: false, error: "season_year must be a positive integer" });
      return;
    }
    if (req.query.week_number !== undefined && !weekNumber) {
      res.status(400).json({ ok: false, error: "week_number must be a positive integer" });
      return;
    }
    const status =
    ...
    if (status && !["draft", "approved", "published", "archived"].includes(status)) {
      res.status(400).json({ ok: false, error: "status must be draft, approved, published, or archived" });
      return;
    }
    let query: any = getServiceSupabase()
      .from("nfl_weekly_slate_templates")
      .select(WEEKLY_SLATE_SELECT)
    ...
    const { data, error } = await query;
    if (weeklySlateDbError(res, error, "Could not list NFL weekly master slates")) return;
    res.json({ ok: true, slates: (data ?? []).map(serializeWeeklySlate) });
  });

  app.get("/api/gameday/admin/nfl-weekly-slates/:slateId", async (req: Request, res: Response) => {
    if (!checkWeeklySlateAdmin(req, res)) return;
    const { data, error } = await getServiceSupabase()
      .from("nfl_weekly_slate_templates")
      .select(WEEKLY_SLATE_SELECT)
      ...
      .maybeSingle();
    if (weeklySlateDbError(res, error, "Could not read NFL weekly master slate")) return;
    if (!data) {
      res.status(404).json({ ok: false, error: "NFL weekly master slate not found" });
      return;
    }
    res.json({ ok: true, slate: serializeWeeklySlate(data) });
  });

  app.patch("/api/gameday/admin/nfl-weekly-slates/:slateId", async (req: Request, res: Response) => {
    if (!checkWeeklySlateAdmin(req, res)) return;
    const supabase = getServiceSupabase();
    const { data: existing, error: readError } = await supabase
      ...
      .eq("id", req.params.slateId)
      .maybeSingle();
    if (weeklySlateDbError(res, readError, "Could not read NFL weekly master slate")) return;
    if (!existing) {
      res.status(404).json({ ok: false, error: "NFL weekly master slate not found" });
      return;
    }
    ...
    if (requestedStatus && requestedStatus !== "draft") {
      res.status(400).json({ ok: false, error: "PATCH status may only be draft" });
      return;
    }
    ...
    if (body.slate_name !== undefined) {
      const slateName = normalizeWeeklySlateText(body.slate_name, 200);
      if (!slateName) {
        res.status(400).json({ ok: false, error: "slate_name must be valid text" });
        return;
      }
      updates.slate_name = slateName;
    }
    if (body.slate_label !== undefined) {
      const slateLabel =
        body.slate_label === null ? null : normalizeWeeklySlateText(body.slate_label, 200);
  ...
  app.post("/api/gameday/admin/nfl-weekly-slates/:slateId/approve", async (req: Request, res: Response) => {
    if (!checkWeeklySlateAdmin(req, res)) return;
    const supabase = getServiceSupabase();
    const { data: existing, error: readError } = await supabase
      .from("nfl_weekly_slate_templates")
    ...
    const { data, error } = await supabase
      ...
      .select(WEEKLY_SLATE_SELECT)
      .single();
    if (weeklySlateDbError(res, error, "Could not approve NFL weekly master slate")) return;
    res.json({ ok: true, slate: serializeWeeklySlate(data) });
  });

  app.post("/api/gameday/admin/nfl-weekly-slates/:slateId/archive", async (req: Request, res: Response) => {
    if (!checkWeeklySlateAdmin(req, res)) return;
    const supabase = getServiceSupabase();
    const { data: existing, error: readError } = await supabase
      .from("nfl_weekly_slate_templates")
    ...
    if (existing.status === "archived") {
      res.json({ ok: true, slate: serializeWeeklySlate(existing) });
      return;
    }
    ...
    const { data, error } = await supabase
      ...
      .select(WEEKLY_SLATE_SELECT)
      .single();
    if (weeklySlateDbError(res, error, "Could not archive NFL weekly master slate")) return;
    res.json({ ok: true, slate: serializeWeeklySlate(data) });
  });

  // ── POST /api/gameday/admin/nfl-weekly-slates/:slateId/publish ───────────
  // This creates private Game Day room instances and post-ready payloads.
  // It intentionally does not call Discord or create message IDs.
  app.post("/api/gameday/admin/nfl-weekly-slates/:slateId/publish", async (req: Request, res: Response) => {
    if (!checkWeeklySlateAdmin(req, res)) return;
    const body = (req.body ?? {}) as Record<string, unknown>;
    const dryRun = body.dry_run === undefined ? false : body.dry_run;
    if (typeof dryRun !== "boolean") {
      res.status(400).json({ ok: false, error: "dry_run must be a boolean" });
      return;
    }
    const guildFilter = normalizeWeeklySlatePublishGuildIds(body.guild_ids);
    if (guildFilter.error) {
      res.status(400).json({ ok: false, error: guildFilter.error });
      return;
    }
    ...
    const { data: slate, error: slateError } = await supabase
      ...
      .eq("id", req.params.slateId)
      .maybeSingle();
    if (weeklySlateDbError(res, slateError, "Could not read NFL weekly master slate")) return;
    if (!slate) {
      res.status(404).json({ ok: false, error: "NFL weekly master slate not found" });
      return;
    }
    ...
    const { data: subscriptions, error: subscriptionsError } = await supabase
      .from("discord_gameday_subscriptions")
      .select("id, discord_guild_id, discord_guild_name, game_day_channel_id, game_day_channel_name, receipt_channel_id, receipt_channel_name, reward_text, status")
      .order("created_at", { ascending: true });
    if (subscriptionsError) {
      ...
      if (isMissingDiscordNflSlateSubscriptionSchemaError(subscriptionsError)) {
        ...
        res.status(500).json({ ok: false, error: "Could not load Discord NFL Slate subscriptions" });
      }
      return;
    }

    const allSubscriptions = (subscriptions ?? []).filter((subscription: any) => {
      if (!guildFilter.provided) return true;
      return guildFilter.ids.includes(subscription.discord_guild_id);
    });
    const activeSubscriptions = allSubscriptions.filter((subscription: any) => subscription.status === "active");
    const inactiveSkipped = allSubscriptions.filter((subscription: any) => subscription.status !== "active").length;

    const { data: existingInstances, error: instancesError } = await supabase
      .from("nfl_weekly_slate_room_instances")
      .select("*")
      .eq("master_slate_id", slate.id);
    if (instancesError) {
      ...
      if (isMissingWeeklySlateSchemaError(instancesError)) {
        ...
        res.status(500).json({ ok: false, error: "Could not load NFL weekly slate room instances" });
      }
      return;
    }
    const instanceByGuild = new Map<string, any>(
      (existingInstances ?? []).map((instance: any) => [instance.discord_guild_id, instance]),
    );
    ...
    const targets: any[] = [];
    const failures: any[] = [];
    const postReadyPayloads: any[] = [];
    const successfulInstances: any[] = [];
    ...
    for (const subscription of activeSubscriptions) {
      ...
      try {
        const roomName = slate.slate_label
          ? `${slate.slate_name} — ${slate.slate_label}`
          : slate.slate_name;
        ...
        const { data: instance, error: instanceInsertError } = await supabase
          .from("nfl_weekly_slate_room_instances")
          .insert({
            master_slate_id: slate.id,
        ...
        if (instanceInsertError) {
          if (instanceInsertError.code === "23505") {
            ...
            const { data: concurrentInstance } = await supabase
              .from("nfl_weekly_slate_room_instances")
              .select("*")
              .eq("master_slate_id", slate.id)
              .eq("discord_guild_id", guildId)
  ...
  app.post("/api/gameday/rooms", async (req: Request, res: Response) => {
    ...
    const {
      ...
      is_private?: boolean;
       sport?: "nba" | "soccer" | "nfl" | "madden";
       template_type?: "nfl_single_game" | "nfl_sunday_slate" | "weekly_pick_card";
    ...
    if (!["nba", "soccer", "nfl", "madden"].includes(normalizedSport)) {
      res.status(400).json({ error: "sport must be nba, soccer, nfl, or madden" });
      return;
    }
    ...
    const requestedTemplateType = typeof template_type === "string" ? template_type.trim().toLowerCase() : "";
     if (requestedTemplateType && !["nfl_single_game", "nfl_sunday_slate", "weekly_pick_card"].includes(requestedTemplateType)) {
       res.status(400).json({ error: "template_type must be nfl_single_game, nfl_sunday_slate, or weekly_pick_card" });
      return;
    }
     const isWeeklyPickCard = requestedTemplateType === "weekly_pick_card";
    if (normalizedSport === "madden" && !isWeeklyPickCard) {
      res.status(400).json({ error: "Madden rooms must use template_type=weekly_pick_card" });
      return;
    }
     ...
     if (isWeeklyPickCard && normalizedSport !== "madden") {
       res.status(400).json({ error: "weekly_pick_card is only supported for Madden rooms" });
       return;
     }
     ...
     if (isWeeklyPickCard) {
       ...
       if (!weeklyRoomName || weeklyRoomName.length > 120) {
         res.status(400).json({ error: "Challenge/week name is required and must be 120 characters or fewer" });
         return;
       }
       if (!matchups) {
         res.status(400).json({ error: "At least one valid Madden matchup with two different teams is required" });
         return;
       }
       if (!weeklyConfig) {
         res.status(400).json({
       ...
       const cardTitle = weeklyConfig.week_label
         ? `${weeklyConfig.week_label} Matchups`
         : "Madden Weekly Matchups";
       const { data: weeklyCard, error: weeklyCardError } = await supabase
         .from("gameday_pick_cards")
         .insert({
       ...
       if (weeklyCardError || !weeklyCard) {
         await supabase.from("gameday_rooms").delete().eq("id", weeklyRoom.id);
         console.error("[gameday] Madden weekly card error:", weeklyCardError);
         res.status(500).json({ error: "Could not create Madden Weekly Pick Card" });
         return;
       }
       ...
       if (weeklyPropError) {
         ...
         res.status(500).json({ error: "Could not create Madden matchup props" });
         return;
       }

       await logEvent(supabase, weeklyRoom.id, null, hostId, "room_created", {
      ...
      for (let i = 0; i < templateProps.length; i++) {
        ...
        const { error: propError } = await supabase.from("gameday_props").insert({
          ...
          answer_options: isSundaySlate
            ? resolveSundaySlateAnswers(tmpl.answers, vars, normalizedSlateConfig!, cardDef.phase)
  ...
  // ── GET /api/gameday/rooms/:roomRef ─────────────────────────────────────
  // Public room references accept either the canonical UUID or GDS short code.
  app.get(
    ...
    async (req: Request, res: Response) => {
      ...
      if (
        error?.message?.includes("template_type") ||
        error?.message?.includes("slate_config") ||
        error?.message?.includes("format_config")
      ) {
  ...
  // ── GET /api/gameday/bot/rooms/current ─────────────────────────────────
  // Recovers one action-eligible Discord room after a bot restart. A channel
  // scopes discovery, but authorization still depends on the stored guild ID.
  app.get(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!channelId) {
        res.status(400).json({ error: "discord_channel_id is required" });
        return;
      }
      ...
      if (action !== "lock" && action !== "resolve" && action !== "final") {
        res.status(400).json({ error: "action must be lock, resolve, or final" });
        return;
      }
      ...
      if (error) {
        ...
        res.status(500).json({ error: "Current room lookup failed" });
        return;
      }

      const isEligible = (room: any) => {
        const cards = Array.isArray(room.gameday_pick_cards)
          ? room.gameday_pick_cards
          : [];
        if (action === "lock") {
          return cards.some((card: any) => card.status === "open");
        }

        const requiredProps = cards.flatMap((card: any) =>
          (Array.isArray(card.gameday_props) ? card.gameday_props : [])
            .filter((prop: any) => isRequiredWeeklyPickCardProp(prop))
            .map((prop: any) => ({ ...prop, card_status: card.status })),
        );
        if (action === "resolve") {
          return requiredProps.some(
            (prop: any) =>
              prop.card_status === "locked" && prop.status !== "settled",
          );
        }

        // Finalization is offered only when this room has playable results and
        // every required prop is settled. The route remains authoritative.
        return (
          requiredProps.length > 0 &&
          requiredProps.every((prop: any) => prop.status === "settled")
        );
      };

      const candidates = (rooms ?? [])
        .filter((room: any) => isEligible(room))
        .map((room: any) => ({
          id: room.id,
        ...
        .sort((a: any, b: any) =>
          String(a.room_code ?? a.id).localeCompare(String(b.room_code ?? b.id)),
        );
      ...
      res.json({ ok: true, room: candidates[0] });
    },
  );

  // ── GET /api/gameday/bot/rooms/:roomRef/settlement ──────────────────────
  ...
  // ── PATCH /api/gameday/rooms/:roomRef/finalize ──────────────────────────
  app.patch(
